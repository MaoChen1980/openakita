...
OpenAkita 内置完整的自维护系统，自动运行：

| 功能 | 执行时间 | 描述 |
|------|----------|------|
| **每日记忆整理** | 凌晨 3:00 | 分析当天对话，提取关键记忆，刷新 MEMORY.md |
| **每日系统自检** | 凌晨 4:00 | 扫描 ERROR 日志，尝试自动修复工具问题，生成报告 |
| **任务复盘** | 长时间任务后 | 回顾任务执行过程，识别问题，记录经验教训 |
| **循环检测** | 实时 | 检测并停止重复的工具调用（防止无限循环） |
